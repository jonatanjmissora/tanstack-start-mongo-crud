...
export const getAllElectrodomesticos = () => {
    ...
    return electrodomesticosData.flatMap(pageData => {
        return pageData.electrodomesticos?.filter(electrodomestico => electrodomestico) || [];
    });
} 

export const searchElectrodomesticos = (params: {page: number, filter: string, sort: "asc" | "desc"}) => {
    const { page, filter, sort } = params;
    
    const filterLower = filter.trim().toLowerCase();
    
    const filteredElectrodomesticos = electrodomesticosData.flatMap(pageData => {
        return pageData.electrodomesticos?.filter(electrodomestico => {
            return electrodomestico?.name?.toLowerCase().includes(filterLower) ?? false;
        }) || [];
    });

    const sortedAndFilteredElectodomesticos = sort === "asc"
