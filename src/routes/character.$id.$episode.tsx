...
export const Route = createFileRoute('/character/$id/$episode')({
  ...
  loader: async ({ params: {episode} }) => {
    const data = await fetch(`https://rickandmortyapi.com/api/episode/${episode}`)
...
function RouteComponent() {
  ...
  return <article className="bg-gray-400 flex flex-col gap-2 p-2">
    <h2 className="text-2xl font-bold">EP {episode.id} {episode.name}</h2>
