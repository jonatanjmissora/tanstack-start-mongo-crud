...
export const Route = createFileRoute('/products/$productId/')({
  ...
  loader: async ({params}) => {
    ...
    const product = products.find((product: ProductType) => product.id === Number(params.productId))
    if (!product) {
      throw new Error('Product not found')
    }
    const productsByCategory = products.filter((productByCategory: ProductType) => (productByCategory.category === product.category) && (productByCategory.id !== product.id))
...
function RouteComponent() {
  ...
  return <article className='p-10 flex flex-col gap-20  items-center'>
    <Link to="/products" className='text-lg underline font-bold mr-auto'>Volver</Link>
